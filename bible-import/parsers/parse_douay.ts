...
const VERSION = 'DRA';
const INPUT_DIR = path.join(__dirname, '../downloads/douay_rheims');
const OUTPUT_FILE = path.join(__dirname, '../output/douay.jsonl');
...
async function main() {
    ...
    let count = 0;
    // Note: uses catholic73 mode
    await parseUsfmFiles(files, { version: VERSION, mode: 'catholic73' }, async (verse) => {
