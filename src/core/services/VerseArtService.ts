import type { RenderConfig, WorkerMessage } from "../pwa/verseArt.worker";
...
class VerseArtService {
  ...
  private initWorker() {
    if (!this.worker && typeof window !== "undefined") {
      this.worker = new Worker(
        new URL("../workers/verseArt.worker.ts", import.meta.url),
        {
          type: "module",
        }
      );
  ...
  public initCanvas(canvasElement: HTMLCanvasElement) {
    ...
    if (this.canvas === canvasElement && this.worker) {
      console.log(
        "[VerseArtService] Already initialized with this canvas. Skipping re-init."
      );
      return;
    }
    ...
    try {
      ...
      if (e.message && e.message.includes("transfer control")) {
        console.warn(
          "[VerseArtService] Canvas already transferred. Ignoring Re-Init."
        );
